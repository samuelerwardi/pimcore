...
class CartPriceCalculatorFactory implements ICartPriceCalculatorFactory
{
    ...
    public function create(IEnvironment $environment, ICart $cart): ICartPriceCalculator
    {
        ...
        return new $class($environment, $cart, $this->createModificators());
    }

    protected function createModificators(): array
    {
        $modificators = [];
        foreach ($this->modificatorConfig as $modificatorConfig) {
            $class = $modificatorConfig['class'];
            $modificators[] = new $class($modificatorConfig['options']);
        }
