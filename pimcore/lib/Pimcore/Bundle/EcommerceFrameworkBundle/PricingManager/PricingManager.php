...
class PricingManager implements IPricingManager
{
    ...
    public function applyCartRules(ICart $cart)
    {
        ...
        $environment->setCart($cart);
        $environment->setExecutionMode(IEnvironment::EXECUTION_MODE_CART);
        ...
        $environment->setCategories(array_values($categories));
        ...
        foreach ($this->getValidRules() as $rule) {
            ...
            // test rule
            if ($rule->check($environment) === false) {
                continue;
            }

            // execute rule
            $rule->executeOnCart($environment);
